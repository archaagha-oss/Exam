...
router.post('/exams/:id/invites/generate', authenticate, isTeacher, async (req: Request, res: Response) => {
  const canManage = await canManageExam(req.user.sub, req.user.role, req.params.id);
  if (!canManage) { res.status(403).json({ error: 'Access denied' }); return; }
  ...
  const results: { studentId: string; email: string; name: string; token: string; magicLink: string }[] = [];
...
router.post('/exams/:id/invites/send-all', authenticate, isTeacher, async (req: Request, res: Response) => {
  const canManage = await canManageExam(req.user.sub, req.user.role, req.params.id);
  if (!canManage) { res.status(403).json({ error: 'Access denied' }); return; }

  const exam = await prisma.exam.findUnique({ where: { id: req.params.id }, select: { title: true } });
  if (!exam) { res.status(404).json({ error: 'Exam not found' }); return; }
  ...
  for (const invite of invites) {
    ...
    await sendInviteEmail(invite.student.email, invite.student.name, exam.title, magicLink, invite.expiresAt).catch(() => {});
    sent++;
  }
...
router.get('/join/:token', async (req: Request, res: Response) => {
  const invite = await prisma.examInvite.findUnique({
    ...
    include: {
      exam: { select: { id: true, title: true, securityLevel: true, ipAllowlist: true, requireOtp: true, status: true } },
      student: { select: { id: true, name: true, email: true } },
    },
  });

  if (!invite) { res.status(404).json({ error: 'Invalid or expired link' }); return; }
  if (invite.usedAt) { res.status(410).json({ error: 'This link has already been used', usedAt: invite.usedAt }); return; }
  if (new Date() > invite.expiresAt) { res.status(410).json({ error: 'This link has expired', expiresAt: invite.expiresAt }); return; }
  if (!['PUBLISHED', 'ACTIVE'].includes(invite.exam.status)) {
    res.status(403).json({ error: 'This exam is not currently available' }); return;
  }
  ...
  res.json({
    data: {
      ...
      // Return a short-lived pre-auth token the student uses to log in
      preAuthToken: crypto.createHmac('sha256', process.env.JWT_SECRET ?? 'dev-secret')
        .update(`${invite.studentId}:${invite.examId}:${Date.now()}`)
        .digest('hex'),
    },
  });
});
...
router.post('/sessions/:sessionId/otp/send', authenticate, isStudent, async (req: Request, res: Response) => {
  ...
  if (!session.exam.requireOtp) { res.status(400).json({ error: 'OTP not required for this exam' }); return; }
  if (session.otpVerifiedAt) { res.status(400).json({ error: 'OTP already verified' }); return; }
...
router.post('/sessions/:sessionId/otp/verify', authenticate, isStudent, async (req: Request, res: Response) => {
  ...
  if (!session || session.studentId !== req.user.sub) { res.status(403).json({ error: 'Forbidden' }); return; }
  if (session.otpVerifiedAt) { res.json({ data: { verified: true } }); return; }

  const otp = await prisma.examOtp.findUnique({ where: { sessionId: req.params.sessionId } });
  if (!otp) { res.status(400).json({ error: 'No OTP found — request a new one' }); return; }
  if (otp.verifiedAt) { res.json({ data: { verified: true } }); return; }
  if (new Date() > otp.expiresAt) { res.status(400).json({ error: 'OTP expired — request a new one', code: 'EXPIRED' }); return; }
  if (otp.attempts >= 3) { res.status(429).json({ error: 'Too many attempts — request a new OTP', code: 'TOO_MANY_ATTEMPTS' }); return; }

  const valid = await bcrypt.compare(parsed.data.code, otp.code);
  if (!valid) {
    await prisma.examOtp.update({ where: { sessionId: req.params.sessionId }, data: { attempts: { increment: 1 } } });
  ...
  const now = new Date();
  await Promise.all([
    prisma.examOtp.update({ where: { sessionId: req.params.sessionId }, data: { verifiedAt: now } }),
    prisma.examSession.update({ where: { id: req.params.sessionId }, data: { otpVerifiedAt: now } }),
  ]);
...
router.post('/sessions/:sessionId/fingerprint', authenticate, isStudent, async (req: Request, res: Response) => {
  const schema = z.object({
  ...
  const knownFingerprints = new Set(previousSessions.map(s => s.deviceFingerprint).filter(Boolean));
...
router.get('/sessions/:sessionId/device-alerts', authenticate, async (req: Request, res: Response) => {
  const session = await prisma.examSession.findUnique({
    where: { id: req.params.sessionId },
    select: { examId: true },
  });
...
router.get('/exams/:id/ip-check', async (req: Request, res: Response) => {
  ...
  if (!exam.ipAllowlist) { res.json({ data: { allowed: true } }); return; }
...
async function sendInviteEmail(email: string, name: string, examTitle: string, magicLink: string, expiresAt: Date) {
