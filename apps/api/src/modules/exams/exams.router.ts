...
router.get('/:id', isTeacher, async (req: Request, res: Response) => {
  const exam = await getExam(req.params.id, req.user.schoolId!);
  if (!exam) {
...
router.put('/:id', isTeacher, async (req: Request, res: Response) => {
  ...
  try {
    const exam = await updateExam(req.params.id, req.user.schoolId!, parsed.data);
    res.json({ data: exam });
  } catch (err: any) {
...
router.delete('/:id', isTeacher, async (req: Request, res: Response) => {
  try {
    await deleteExam(req.params.id, req.user.schoolId!);
...
router.post('/:id/publish', isTeacher, async (req: Request, res: Response) => {
  try {
    const exam = await publishExam(req.params.id, req.user.schoolId!);
    res.json({ data: exam });
  } catch (err: any) {
    if (err?.status === 404 || err?.name === 'NotFoundError') {
      res.status(404).json({ error: 'Exam not found' });
...
router.post('/:id/assign', isTeacher, async (req: Request, res: Response) => {
  ...
  try {
    await assignExam(req.params.id, req.user.schoolId!, parsed.data.classIds);
...
router.post('/:id/items', isTeacher, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (err?.status === 404 || err?.name === 'NotFoundError') {
      res.status(404).json({ error: 'Exam or question not found' });
      return;
    }
    throw err;
  }
...
router.delete('/:id/items/:itemId', isTeacher, async (req: Request, res: Response) => {
  try {
    await removeExamItem(req.params.itemId, req.user.schoolId!);
...
router.put('/:id/items/:itemId', isTeacher, async (req: Request, res: Response) => {
  ...
  // Tenant check: item must belong to an exam in the caller's school
  const owned = await prisma.examItem.findFirst({
    where: { id: req.params.itemId, exam: { schoolId: req.user.schoolId! } },
    select: { id: true },
  });
  if (!owned) {
    res.status(404).json({ error: 'Item not found' });
...
router.put('/:id/items/reorder', isTeacher, async (req: Request, res: Response) => {
  ...
  try {
    await reorderExamItems(req.params.id, req.user.schoolId!, parsed.data.itemIds);
    res.json({ data: { message: 'Items reordered' } });
  } catch (err: any) {
    if (err?.status === 404 || err?.name === 'NotFoundError') {
      res.status(404).json({ error: 'Exam or item not found' });
      return;
    }
    throw err;
  }
...
router.post('/:id/activate', isTeacher, async (req: Request, res: Response) => {
  ...
  await audit(req.user.sub, 'EXAM_PUBLISHED', 'Exam', exam.id, {
    title: exam.title,
    status: 'ACTIVE',
  });
  res.json({ data: exam });
