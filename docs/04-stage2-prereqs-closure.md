...
- ~~**SQL `CHECK` constraint** enforcing role↔schoolId pairing.~~ **Shipped in cycle 4.0** (`apps/api/prisma/migrations/20260510150000_5_role_schoolid_check_constraint/`). PLATFORM_ADMIN with NULL schoolId, every other role with non-NULL — enforced at the database level on top of the application-layer guard.
...
- ~~**Bulk back-fill of `actorRole` / `actorSchoolId` for pre-existing audit rows.**~~ **Shipped in cycle 4.0** (`apps/api/scripts/backfill-audit-actor-meta.ts`). Reads each row with NULL `actorRole`, joins to `users`, fills `actorRole` / `actorSchoolId` / `impersonation` (computed from existing `targetSchoolId`). Idempotent + supports `--dry-run`. Greenfield → no real rows to backfill yet; the script is in the tree for the day a real customer lands.
