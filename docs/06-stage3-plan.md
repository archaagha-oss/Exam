...
| --- | --- | --- | --- |
| **3.0a** *(this cycle)* | Stage 3 prep + abstractions | D7 ratified in `decisions.md`. This plan doc. Postgres pool size from env. WS broadcast abstraction (`lib/wsBroadcast.ts`) used by all current callers; in-process impl today. Load-test scaffolding (`scripts/load-test/`) ready to run at any concurrency. | Low — no behaviour change. |
| **3.0b** | Redis pub/sub WS fan-out | `lib/wsBroadcast.ts` swaps to a Redis pub/sub implementation. Each API replica subscribes to the channel and routes incoming messages to its local WS clients. Sticky-session-friendly: a student connects to whichever replica nginx routes them to. Tests against a 2-replica local stack. | Medium — touches the hot WS path; needs a soak test. |
| **3.0c** | HTTP-only autosave authoritative + load-test baseline | The current `session:answer` WS message is duplicative with the REST POST. Make REST authoritative, WS optimistic-only (still useful for the proctor-update side-effect, but not for durability). Run the load-test against a 2-replica local stack and a single-replica baseline. Document numbers in `docs/07-stage3-closure.md`. | Medium — small surface area but changes the durability invariant. |
| **3.0d** *(closure)* | Stage 3 closure | `docs/07-stage3-closure.md` mapping the cycle plan to what shipped, the load-test numbers vs the 5k target, and the `chaos-test` matrix updated for the multi-replica topology. | Low — doc only. |
